...
public class SystemSwitch<T> extends Switch<T> {
    ...
    protected T doSwitch(final int classifierID, final EObject theEObject) {
        ...
        case SystemPackage.INDIRECTIONS_AWARE_SYSTEM: {
            final IndirectionsAwareSystem indirectionsAwareSystem = (IndirectionsAwareSystem) theEObject;
            T result = this.caseIndirectionsAwareSystem(indirectionsAwareSystem);
            if (result == null) {
                result = this.caseSystem(indirectionsAwareSystem);
            }
            if (result == null) {
                result = this.caseComposedProvidingRequiringEntity(indirectionsAwareSystem);
            }
            if (result == null) {
                result = this.caseIdentifier(indirectionsAwareSystem);
            }
            if (result == null) {
                result = this.caseNamedElement(indirectionsAwareSystem);
            }
            if (result == null) {
                result = this.caseComposedStructure(indirectionsAwareSystem);
            }
            if (result == null) {
                result = this.caseInterfaceProvidingRequiringEntity(indirectionsAwareSystem);
            }
            if (result == null) {
                result = this.caseInterfaceProvidingEntity(indirectionsAwareSystem);
            }
            if (result == null) {
                result = this.caseInterfaceRequiringEntity(indirectionsAwareSystem);
            }
            if (result == null) {
                result = this.caseResourceInterfaceRequiringEntity(indirectionsAwareSystem);
            }
            if (result == null) {
                result = this.caseEntity(indirectionsAwareSystem);
            }
            if (result == null) {
                result = this.defaultCase(theEObject);
            }
     ...
     * System</em>'. <!-- begin-user-doc --> This implementation returns null; returning a non-null
     * result will terminate the switch. <!-- end-user-doc -->
     *
     * @param object
     *            the target of the switch.
     ...
     * @generated
     */
    public T caseIndirectionsAwareSystem(final IndirectionsAwareSystem object) {
        return null;
    }
